...
import { Bell, CheckCircle, AlertCircle, Info, Clock, ArrowRight, Loader2, Inbox } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useGetNotificationsQuery, useMarkAllAsReadMutation, useMarkAsReadMutation } from '../redux/features/notification';
            ...
            <div className="space-y-6 max-w-5xl mx-auto">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        ...
                        <p className="text-gray-500 font-medium mt-1">Stay updated with platform activities and alerts</p>
                    </div>
                ...
                <div className="space-y-5">
                    {displayedNotifications.map((notif: any) => (
                        <Card
                            key={notif._id}
                            noPadding
                            className={`group transition-all duration-500 hover:shadow-2xl border-l-[6px] cursor-pointer overflow-hidden ${notif.isRead ? 'border-transparent bg-gray-50/50 opacity-80 hover:opacity-100' : 'border-[#E4983A] bg-white ring-1 ring-orange-100/50'}`}
                            onClick={() => handleNotificationClick(notif)}
                        >
                            <div className="p-6 flex items-start gap-6">
                                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-sm transition-transform group-hover:scale-110 duration-500 ${getBgColor(notif.title)}`}>
                                    {getIcon(notif.title)}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                                        <h3 className={`text-xl font-black tracking-tight ${notif.isRead ? 'text-gray-600' : 'text-gray-900'}`}>
                                            {notif.title}
                                        </h3>
                                    ...
                                    <p className={`leading-relaxed font-semibold transition-colors duration-500 ${notif.isRead ? 'text-gray-500' : 'text-gray-700'}`}>
                                        {notif.message}
                                    </p>

                                    {(notif.orderId || notif.userId?._id) && (
                                        <div className="mt-4 text-sm font-black text-[#E4983A] flex items-center gap-2 group/btn">
                                            {notif.orderId ? 'View Order Details' : 'View Profile Details'}
                                            <div className="p-1 bg-orange-50 rounded-lg group-hover:bg-[#E4983A] group-hover:text-white transition-colors">
                                ...
                                {!notif.isRead && (
                                    <div className="w-4 h-4 rounded-full bg-[#E4983A] mt-2 shadow-lg shadow-orange-200 ring-4 ring-orange-50 animate-pulse shrink-0" title="Unread"></div>
                                )}
                            </div>
                        </Card>
                    ))}
                    ...
                    {displayedNotifications.length === 0 && (
                        <Card className="flex flex-col items-center justify-center py-24 text-center border-none shadow-xl">
                            ...
                            <h2 className="text-2xl font-black text-gray-900 mb-2">Perfectly caught up!</h2>
                            <p className="text-gray-500 font-medium max-w-xs">You have no {filter === 'unread' ? 'unread' : ''} notifications at the moment.</p>
                        </Card>
                    )}
                </div>
